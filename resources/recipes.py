...
def generate(rm: ResourceManager):
    ...
    for wood in ('acacia', 'oak', 'dark_oak', 'jungle', 'birch', 'spruce', 'crimson', 'warped'):
        if wood == 'crimson' or wood == 'warped':
            ...
            name = '%s_log' % wood
        tool_damaging_shaped(rm, '%s_planks_with_saw' % wood, ('S', 'W'), {'S': '#notreepunching:saws', 'W': '#minecraft:%ss' % name}, (4, 'minecraft:%s_planks' % wood)).with_advancement('minecraft:%s' % name)
        tool_damaging_shaped(rm, '%s_planks_with_flint_axe' % wood, ('S', 'W'), {'S': '#notreepunching:weak_saws', 'W': '#minecraft:%ss' % name}, (2, 'minecraft:%s_planks' % wood)).with_advancement('minecraft:%s' % name)

    # Sticks
    tool_damaging_shaped(rm, 'sticks_from_logs_with_saw', ('SW',), {'S': '#notreepunching:saws', 'W': '#minecraft:logs'}, (8, 'minecraft:stick')).with_advancement('#minecraft:logs')
    tool_damaging_shaped(rm, 'sticks_from_planks_with_saw', ('SW',), {'S': '#notreepunching:saws', 'W': '#minecraft:planks'}, (2, 'minecraft:stick')).with_advancement('#minecraft:planks')

    tool_damaging_shaped(rm, 'sticks_from_logs_with_flint_axe', ('SW',), {'S': '#notreepunching:weak_saws', 'W': '#minecraft:logs'}, (6, 'minecraft:stick')).with_advancement('#minecraft:logs')
    tool_damaging_shaped(rm, 'sticks_from_planks_with_flint_axe', ('SW',), {'S': '#notreepunching:weak_saws', 'W': '#minecraft:planks'}, (1, 'minecraft:stick')).with_advancement('#minecraft:planks')

    # Tools
    for tool in ('iron', 'gold', 'diamond'):
        ingot = '#forge:ingots/%s' % tool if tool != 'diamond' else '#forge:gems/diamond'
        rm.crafting_shaped('%s_knife' % tool, ('I', 'S'), {'S': '#forge:rods/wooden', 'I': ingot}, 'notreepunching:%s_knife' % tool).with_advancement(ingot)
        rm.crafting_shaped('%s_mattock' % tool, ('III', ' SI', ' S '), {'S': '#forge:rods/wooden', 'I': ingot}, 'notreepunching:%s_mattock' % tool).with_advancement(ingot)
        rm.crafting_shaped('%s_saw' % tool, ('  S', ' SI', 'SI '), {'S': '#forge:rods/wooden', 'I': ingot}, 'notreepunching:%s_saw' % tool).with_advancement(ingot)
    ...
    # Flint Tools
    rm.crafting_shaped('flint_axe', ('PI', 'S '), {'S': '#forge:rods/wooden', 'I': 'notreepunching:flint_shard', 'P': '#forge:string'}, 'notreepunching:flint_axe').with_advancement('notreepunching:flint_shard')
    rm.crafting_shaped('flint_hoe', ('PII', 'S  ', 'S  '), {'S': '#forge:rods/wooden', 'I': 'notreepunching:flint_shard', 'P': '#forge:string'}, 'notreepunching:flint_hoe').with_advancement('notreepunching:flint_shard')
    rm.crafting_shaped('flint_knife', ('I', 'S'), {'S': '#forge:rods/wooden', 'I': 'notreepunching:flint_shard'}, 'notreepunching:flint_knife').with_advancement('notreepunching:flint_shard')
    rm.crafting_shaped('flint_pickaxe', ('IPI', 'ISI', ' S '), {'S': '#forge:rods/wooden', 'I': 'notreepunching:flint_shard', 'P': '#forge:string'}, 'notreepunching:flint_pickaxe').with_advancement('notreepunching:flint_shard')
    rm.crafting_shaped('flint_shovel', (' II', ' PI', 'S  '), {'S': '#forge:rods/wooden', 'I': 'notreepunching:flint_shard', 'P': '#forge:string'}, 'notreepunching:flint_shovel').with_advancement('notreepunching:flint_shard')
    rm.crafting_shaped('macuahuitl', (' IS', 'ISI', 'SI '), {'S': '#forge:rods/wooden', 'I': 'notreepunching:flint_shard'}, 'notreepunching:macuahuitl').with_advancement('notreepunching:flint_shard')

    # Misc Tools
    rm.crafting_shaped('clay_tool', ('  I', ' II', 'I  '), {'I': '#forge:rods/wooden'}, 'notreepunching:clay_tool').with_advancement('#forge:rods/wooden')
    rm.crafting_shaped('fire_starter', ('SP', 'FS'), {'S': '#forge:rods/wooden', 'P': '#forge:string', 'F': 'notreepunching:flint_shard'}, 'notreepunching:fire_starter').with_advancement('notreepunching:flint_shard')
    ...
    rm.crafting_shapeless('string_from_wool_with_knife', ('#minecraft:wool', knife), (4, 'minecraft:string')).with_advancement('#minecraft:wool')
    ...
    rm.crafting_shapeless('plant_fiber_from_leaves_with_knife', ('#minecraft:leaves', knife), 'notreepunching:plant_fiber').with_advancement('#minecraft:leaves')
    rm.crafting_shapeless('plant_fiber_from_saplings_with_knife', ('#minecraft:saplings', knife), (2, 'notreepunching:plant_fiber')).with_advancement('#minecraft:saplings')
    rm.crafting_shapeless('plant_fiber_from_small_flowers_with_knife', ('#minecraft:small_flowers', knife), 'notreepunching:plant_fiber').with_advancement('#minecraft:small_flowers')
    rm.crafting_shapeless('plant_fiber_from_tall_flowers_with_knife', ('#minecraft:tall_flowers', knife), (2, 'notreepunching:plant_fiber')).with_advancement('#minecraft:tall_flowers')
    ...
    for wood in ('aspen', 'baobab', 'blue_enchanted', 'cherry', 'cika', 'cypress', 'ebony', 'ether', 'fir', 'green_enchanted', 'holly', 'jacaranda', 'lament', 'mahogany', 'mangrove', 'maple', 'nightshade', 'palm', 'pine', 'rainbow_eucalyptus', 'redwood', 'skyris', 'willow', 'witch_hazel', 'zelkova'):
        tool_damaging_shaped(rm, 'byg:%s_planks' % wood, ('S', 'W'), {'S': '#notreepunching:saws', 'W': '#byg:%s_logs' % wood}, (4, 'byg:%s_planks' % wood), conditions=condition)
        tool_damaging_shaped(rm, 'compat/byg_%s_planks_with_flint_axe' % wood, ('S', 'W'), {'S': '#notreepunching:weak_saws', 'W': '#byg:%s_logs' % wood}, (2, 'byg:%s_planks' % wood), conditions=condition)
