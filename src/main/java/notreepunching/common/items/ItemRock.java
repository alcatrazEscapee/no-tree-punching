...
import alcatrazcore.item.ItemCore;
import alcatrazcore.util.CoreHelpers;
import notreepunching.common.blocks.BlockRock;
import notreepunching.util.Stone;
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
        ...
        if (stateUnder.isNormalCube() && stateAt.getBlock().isReplaceable(worldIn, pos.up()))
        {
            if (!worldIn.isRemote)
            {
                ...
                player.setHeldItem(hand, CoreHelpers.consumeItem(stack));
